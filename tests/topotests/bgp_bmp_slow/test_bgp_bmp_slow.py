...
def setup_module(mod):
    ...
    # Nexthop must be a local address on r2 (its loopback) so the routes resolve
    # and become active -- only active routes are redistributed into BGP.
    tgen.gears["r2"].vtysh_cmd(
        "sharp install routes 100.64.0.0 nexthop 172.16.255.2 {}".format(NROUTES)
    )
    tgen.gears["r2"].vtysh_cmd(
        "sharp install routes 2001:db8:1:: nexthop 2001:db8:ffff::2 {}".format(NROUTES)
    )
...
def test_no_route_monitoring_truncation():
    ...
    topotest.run_and_expect(_settled, True, count=90, wait=2)
